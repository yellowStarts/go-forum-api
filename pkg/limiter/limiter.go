...
// GetKeyRouterWithIP Limitor 的 Key, 路由 + IP，针对单个路由做限流
func GetKeyRouterWithIP(c *gin.Context) string {
...
func CheckRate(c *gin.Context, key string, formatted string) (limiterlib.Context, error) {
	...
	limiterlib := limiterlib.New(store, rate)

	// 获取限流的结果
	return limiterlib.Get(c, key)
}
